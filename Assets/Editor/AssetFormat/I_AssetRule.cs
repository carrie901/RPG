...
namespace SummerEditor
{
    ...
    {
        void SetFilter(EdNode node);

        bool IsMatch<T>(AssetImporter assetImport, T obj) where T : UnityEngine.Object;
    }

    public class AssetImportHelper
    {
        public static List<int> pots = new List<int>() { 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048 };
        public static bool IsMath(string asset_path, string filter_path)
        {
            Debug.Assert(!string.IsNullOrEmpty(asset_path), "路径为空:这是BUG");
            if (string.IsNullOrEmpty(asset_path)) return false;
            if (asset_path.Contains(filter_path)) return true;
            return false;
        }
        ...
        public static bool IsPot(Texture2D tex)
        {
            ...
            if (!pots.Contains(width) || !pots.Contains(height)) return false;
            return true;
        }
