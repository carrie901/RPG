...
public partial class TableView
{
    private void DrawTitle(float width)
    {
        for (int i = 0; i < m_descArray.Count; i++)
        {
            ...
            bool selected = _sort_slot == i;
            GUI.Label(r, desc.title_text + (selected ? _appearance.GetSortMark(_descending) : ""), _appearance.GetTitleStyle(selected));
    ...
    private void SortData()
    {
        m_lines.Sort((s1, s2) =>
        {
            if (_sort_slot >= m_descArray.Count)
                return 0;

            return m_descArray[_sort_slot].Compare(s1, s2) * (_descending ? -1 : 1);
        });
    }
    ...
    private Rect LabelRect(float width, int slot, int pos)
    {
        ...
        {
            accumPercent += m_descArray[i].width_in_percent;
        }
        return new Rect(width * accumPercent, pos * _appearance.LineHeight, width * m_descArray[slot].width_in_percent, _appearance.LineHeight);
    }
