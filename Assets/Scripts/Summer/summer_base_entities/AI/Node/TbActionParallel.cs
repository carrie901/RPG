...
namespace Summer.AI
{
    ...
    /// </summary>
    public class TbActionParallel : TbAction
    {
        ...
        public TbActionParallel() : base(-1)
        {
            _evaluation_relationship = ECHILDREN_RELATIONSHIP.and;
            _runningstatus_relationship = ECHILDREN_RELATIONSHIP.or;
        }
        public TbActionParallel SetEvaluationRelationship(ECHILDREN_RELATIONSHIP v)
        {
            _evaluation_relationship = v;
            return this;
        }
        public TbActionParallel SetRunningStatusRelationship(ECHILDREN_RELATIONSHIP v)
        {
        ...
        protected override int OnUpdate(TbWorkingData work_data)
        {
            ...
            {
                _init_list_to(this_context.running_status, TbRunningStatus.EXECUTING);
            }
            ...
            for (int i = 0; i < GetChildCount(); ++i)
            {
                ...
                if (TbRunningStatus.IsFinished(this_context.running_status[i]))
                {
                    has_finished = true;
                    continue;
                }
