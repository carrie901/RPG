...
namespace Summer
{
    public class EntityFindTargetAction : I_EntityAction
    {
        public void OnAction(BaseEntity entity, EventSetData param)
        {
            ...
            Vector3 world_position = entity.EntityController.WroldPosition;

            int length = EntitesManager.Instance.entites.Count;
            for (int i = 0; i < length; i++)
            {
                BaseEntity controller = EntitesManager.Instance.entites[i];
                if (controller == entity) continue;
                // 2.双方之间的距离
                float distance = MathHelper.Distance2D(controller.EntityController.WroldPosition, world_position);
                // 3.距离大于指定长度
                if (distance > data.radius) continue;
                // 4.自己和目标之间的方向
                Vector3 target_direction = controller.EntityController.WroldPosition - world_position;
                // 5.角度小于指定长度 敌我方向和自身的正前方之间的夹角
                float tmp_angle = MathHelper.GetAngle04(target_direction, direction);
                if (tmp_angle > angle) continue;
                // 添加到目标
                entity._targets.Add(controller);
            }
        }
    }
}
